...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in OctenClaw
...
Running OctenClaw via Node with `tsx` fails at startup with:

```
[octenclaw] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `octenclaw status` and `gateway:watch` worked.
