...
```bash
octenclaw tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `octenclaw tui --deliver`
...
- If your terminal uses a light background and auto-detection is wrong, set `OPENCLAW_THEME=light` before launching `octenclaw tui`.
...
- Check the Gateway logs: `octenclaw logs --follow`.
- Confirm the agent can run: `octenclaw status` and `octenclaw models status`.
...
- No agents in picker: check `octenclaw agents list` and your routing config.
