...
If the page fails to load, start the Gateway first: `octenclaw gateway`.
...
you revoke it with `octenclaw devices revoke --device <id> --role <role>`. See
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop octenclaw`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. OctenClaw
...
```bash
octenclaw gateway --bind tailnet --token "$(openssl rand -hex 32)"
```
...
OctenClaw **blocks** Control UI connections without device identity.
