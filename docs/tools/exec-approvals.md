...
- **gateway host** → `octenclaw` process on the gateway machine
...
- For shell scripts and direct interpreter/runtime file invocations, OctenClaw also tries to bind
...
- allowlist entries live in host-local `~/.openclaw/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `octenclaw approvals allowlist ...`).
- `octenclaw security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `octenclaw doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
CLI: `octenclaw approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
...
These clients are opt-in. If a channel does not have exec approvals enabled, OctenClaw does not treat
...
topics, OctenClaw preserves the topic for the approval prompt and the post-approval follow-up.
