...
read_when:
  - Running OctenClaw Gateway in WSL2 while Chrome lives on Windows
...
- OctenClaw Gateway runs inside WSL2
...
Use the built-in `chrome-relay` profile plus the OctenClaw Chrome extension.
...
- OctenClaw points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, OctenClaw is not the problem yet.
...
Fix that before touching OctenClaw config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point OctenClaw at the address that is reachable from WSL2:
...
- test the same URL with `curl` before expecting OctenClaw to succeed
...
```bash
octenclaw browser open https://example.com --browser-profile remote
octenclaw browser tabs --browser-profile remote
```

For the extension relay:

```bash
octenclaw browser tabs --browser-profile chrome-relay
```

Good result:

- the tab opens in Windows Chrome
- `octenclaw browser tabs` returns the target
...
3. OctenClaw config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug OctenClaw config or Control UI auth
