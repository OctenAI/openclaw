---
summary: "Chrome extension: let OctenClaw drive your existing Chrome tab"
read_when:
...
The OctenClaw Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate octenclaw-managed Chrome profile.

Attach/detach happens via a **single Chrome toolbar button**.

If you want Chrome’s official DevTools MCP attach flow instead of the OctenClaw
...
OctenClaw then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the OctenClaw release (npm package) as static files. There is no separate “build” step.

After upgrading OctenClaw:

- Re-run `octenclaw browser extension install` to refresh the installed files under your OctenClaw state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

OctenClaw ships with a built-in browser profile named `chrome-relay` that targets the extension relay on the default port.
...
- Open the tab you want OctenClaw to control.
...
- `ON`: attached; OctenClaw can drive that tab.
...
Debugging: `octenclaw sandbox explain`
...
`octenclaw browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `octenclaw browser extension install` first to copy the extension to a stable location under your OctenClaw state directory.
...
- **This is not isolated** like the dedicated octenclaw-managed profile.
