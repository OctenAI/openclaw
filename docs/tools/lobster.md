---
title: Lobster
summary: "Typed workflow runtime for OctenClaw with resumable approval gates."
description: Typed workflow runtime for OctenClaw — composable pipelines with approval gates.
read_when:
...
Lobster is a workflow shell that lets OctenClaw run multi-step tool sequences as a single, deterministic operation with explicit approval checkpoints.
...
- **One call instead of many**: OctenClaw runs one Lobster tool call and gets a structured result.
...
OctenClaw launches the local `lobster` CLI in **tool mode** and parses a JSON envelope from stdout.
...
Lobster can run YAML/JSON workflow files with `name`, `args`, `steps`, `env`, `condition`, and `approval` fields. In OctenClaw tool calls, set `pipeline` to the file path.
...
Install the Lobster CLI on the **same host** that runs the OctenClaw Gateway (see the [Lobster repo](https://github.com/openclaw/lobster)), and ensure `lobster` is on `PATH`.
...
plugin tools (like `lobster`), OctenClaw keeps core tools enabled. To restrict core
...
→ octenclaw calls gmail.list
→ LLM summarizes
→ User: "draft replies to #2 and #5"
→ LLM drafts
→ User: "send #2"
→ octenclaw calls gmail.send
...
- **No secrets** — Lobster doesn't manage OAuth; it calls OctenClaw tools that do.
