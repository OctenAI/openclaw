---
summary: "Run OctenClaw Gateway 24/7 on a GCP Compute Engine VM (Docker) with durable state"
read_when:
  - You want OctenClaw running 24/7 on GCP
  - You want a production-grade, always-on Gateway on your own VM
  - You want full control over persistence, binaries, and restart behavior
title: "GCP"
---

# OctenClaw on GCP Compute Engine (Docker, Production VPS Guide)

## Goal

Run a persistent OctenClaw Gateway on a GCP Compute Engine VM using Docker, with durable state, baked-in binaries, and safe restart behavior.

If you want "OctenClaw 24/7 for ~$5-12/mo", this is a reliable setup on Google Cloud.
...
- Start the OctenClaw Gateway in Docker
...
```bash
gcloud projects create my-octenclaw-project --name="OctenClaw Gateway"
gcloud config set project my-octenclaw-project
```
...
```bash
gcloud compute instances create octenclaw-gateway \
  --zone=us-central1-a \
...
```bash
gcloud compute ssh octenclaw-gateway --zone=us-central1-a
```
...
## 6) Clone the OctenClaw repository
...
```bash
OPENCLAW_IMAGE=octenclaw:latest
...
```bash
docker compose run --rm octenclaw-cli config set gateway.controlUi.allowedOrigins '["http://127.0.0.1:18789"]' --strict-json
```
...
```bash
gcloud compute ssh octenclaw-gateway --zone=us-central1-a -- -L 18789:127.0.0.1:18789
```
...
```bash
# Stop the VM first
gcloud compute instances stop octenclaw-gateway --zone=us-central1-a

# Change machine type
gcloud compute instances set-machine-type octenclaw-gateway \
  --zone=us-central1-a \
  --machine-type=e2-small

# Start the VM
gcloud compute instances start octenclaw-gateway --zone=us-central1-a
```
...
1. Create a service account:
   ...
   ```bash
   gcloud iam service-accounts create octenclaw-deploy \
     --display-name="OctenClaw Deployment"
   ```

2. Grant Compute Instance Admin role (or narrower custom role):

   ```bash
   gcloud projects add-iam-policy-binding my-octenclaw-project \
     --member="serviceAccount:octenclaw-deploy@my-octenclaw-project.iam.gserviceaccount.com" \
