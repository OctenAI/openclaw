---
summary: "Deploy OctenClaw Gateway to a Kubernetes cluster with Kustomize"
read_when:
  - You want to run OctenClaw on a Kubernetes cluster
  - You want to test OctenClaw in a Kubernetes environment
title: "Kubernetes"
---

# OctenClaw on Kubernetes

A minimal starting point for running OctenClaw on Kubernetes — not a production-ready deployment. It covers the core resources and is meant to be adapted to your environment.

## Why not Helm?

OctenClaw is a single container with some config files. The interesting customization is in agent content (markdown files, skills, config overrides), not infrastructure templating. Kustomize handles overlays without the overhead of a Helm chart. If your deployment grows more complex, a Helm chart can be layered on top of these manifests.
...
kubectl port-forward svc/openclaw 18789:18789 -n octenclaw
...
```bash
kubectl get secret octenclaw-secrets -n octenclaw -o jsonpath='{.data.OPENCLAW_GATEWAY_TOKEN}' | base64 -d
```
...
```
Namespace: octenclaw (configurable via OPENCLAW_NAMESPACE)
...
```bash
kubectl patch secret octenclaw-secrets -n octenclaw \
  -p '{"stringData":{"<PROVIDER>_API_KEY":"..."}}'
kubectl rollout restart deployment/openclaw -n octenclaw
```
