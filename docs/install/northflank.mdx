...
Deploy OctenClaw on Northflank with a one-click template and finish setup in your browser.
...
1. Click [Deploy OctenClaw](https://northflank.com/stacks/deploy-octenclaw) to open the template.
2. Create an [account on Northflank](https://app.northflank.com/signup) if you don’t already have one.
3. Click **Deploy OctenClaw now**.
4. Set the required environment variable: `SETUP_PASSWORD`.
5. Click **Deploy stack** to build and run the OctenClaw template.
6. Wait for the deployment to complete, then click **View resources**.
7. Open the OctenClaw service.
8. Open the public OctenClaw URL and complete setup at `/setup`.
9. Open the Control UI at `/openclaw`.

## What you get

- Hosted OctenClaw Gateway + Control UI
