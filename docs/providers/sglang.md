---
summary: "Run OctenClaw with SGLang (OpenAI-compatible self-hosted server)"
read_when:
  - You want to run OctenClaw against a local SGLang server
...
OctenClaw can connect to SGLang using the `openai-completions` API.

OctenClaw can also **auto-discover** available models from SGLang when you opt
...
define `models.providers.sglang`, OctenClaw will query:
