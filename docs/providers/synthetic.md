---
summary: "Use Synthetic's Anthropic-compatible API in OctenClaw"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. OctenClaw registers it as the
...
```bash
octenclaw onboard --auth-choice synthetic-api-key
```
...
Note: OctenClaw's Anthropic client appends `/v1` to the base URL, so use
