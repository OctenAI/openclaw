---
summary: "Use OpenCode Zen and Go catalogs with OctenClaw"
read_when:
...
OpenCode exposes two hosted catalogs in OctenClaw:

- `opencode/...` for the **Zen** catalog
- `opencode-go/...` for the **Go** catalog

Both catalogs use the same OpenCode API key. OctenClaw keeps the runtime provider ids
...
octenclaw onboard --opencode-zen-api-key "$OPENCODE_API_KEY"
```

### Go catalog

```bash
octenclaw onboard --auth-choice opencode-go
octenclaw onboard --opencode-go-api-key "$OPENCODE_API_KEY"
```
