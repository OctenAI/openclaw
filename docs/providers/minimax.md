---
summary: "Use MiniMax M2.5 in OctenClaw"
read_when:
  - You want MiniMax models in OctenClaw
...
```bash
octenclaw plugins enable minimax-portal-auth  # skip if already loaded.
octenclaw gateway restart  # restart if gateway is already running
octenclaw onboard --auth-choice minimax-portal
```
...
- Use `octenclaw models list` and `octenclaw models set minimax/MiniMax-M2.5` to switch.
...
- Running `octenclaw configure` and selecting **MiniMax M2.5**, or
