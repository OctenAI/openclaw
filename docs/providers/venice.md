---
summary: "Use Venice AI privacy-focused models in OctenClaw"
read_when:
  - You want privacy-focused inference in OctenClaw
...
```bash
octenclaw onboard --auth-choice venice-api-key
```
...
```bash
octenclaw agent --model venice/kimi-k2-5 --message "Hello, are you working?"
```

## Model Selection

After setup, OctenClaw shows all available Venice models. Pick based on your needs:
...
```bash
octenclaw models set venice/kimi-k2-5
octenclaw models set venice/claude-opus-4-6
```

List all available models:

```bash
octenclaw models list | grep venice
```

## Configure via `octenclaw configure`

1. Run `octenclaw configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
OctenClaw automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
octenclaw agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
octenclaw agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
octenclaw agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
octenclaw agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
octenclaw agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `octenclaw models list` to see currently available models. Some models may be temporarily offline.
