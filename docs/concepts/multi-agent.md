...
If you do nothing, OctenClaw runs a single agent:
...
```bash
octenclaw agents list --bindings
```
...
```bash
octenclaw agents add coding
octenclaw agents add social
```
...
- If you later add the same binding for the same agent with an explicit account id, OctenClaw upgrades the existing channel-only binding to account-scoped instead of duplicating it.
...
`channels.<channel>.defaultAccount` (optional). When unset, OctenClaw falls back
...
```bash
octenclaw channels login --channel whatsapp --account personal
octenclaw channels login --channel whatsapp --account biz
```
