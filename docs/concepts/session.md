...
OctenClaw treats **one direct-chat session per agent** as primary. Direct chats collapse to `agent:<agentId>:<mainKey>` (default `main`), while group/channel chats get their own keys. `session.mainKey` is honored.
...
- You can verify your DM settings with `octenclaw security audit` (see [security](/cli/security)).

## Gateway is the source of truth

All session state is **owned by the gateway** (the “master” OctenClaw). UI clients (macOS app, WebChat, etc.) must query the gateway for session lists and token counts instead of reading local files.
...
- OctenClaw does **not** read legacy Pi/Tau session folders.

## Maintenance

OctenClaw applies session-store maintenance to keep `sessions.json` and transcript artifacts bounded over time.
...
Maintenance runs during session-store writes, and you can trigger it on demand with `octenclaw sessions cleanup`.
...
- run `octenclaw sessions cleanup --dry-run --json` after config changes to verify projected impact before enforcing
...
```bash
octenclaw sessions cleanup --dry-run
octenclaw sessions cleanup --enforce
```

## Session pruning

OctenClaw trims **old tool results** from the in-memory context right before LLM calls by default.
This does **not** rewrite JSONL history. See [/concepts/session-pruning](/concepts/session-pruning).

## Pre-compaction memory flush

When a session nears auto-compaction, OctenClaw can run a **silent memory flush**
...
- Legacy idle-only: if you set `session.idleMinutes` without any `session.reset`/`resetByType` config, OctenClaw stays in idle-only mode for backward compatibility.
...
- `octenclaw status` — shows store path and recent sessions.
- `octenclaw sessions --json` — dumps every entry (filter with `--active <minutes>`).
- `octenclaw gateway call sessions.list --params '{}'` — fetch sessions from the running gateway (use `--url`/`--token` for remote gateway access).
...
- Send `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop octenclaw`) to abort the current run, clear queued followups for that session, and stop any sub-agent runs spawned from it (the reply includes the stopped count).
