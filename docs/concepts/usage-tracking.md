...
- `/usage cost` in chats: local cost summary aggregated from OctenClaw session logs.
- CLI: `octenclaw status --usage` prints a full per-provider breakdown.
- CLI: `octenclaw channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
