...
read_when:
  - You want to place an outbound voice call from OctenClaw
...
Voice calls for OctenClaw via a plugin. Supports outbound notifications and
...
- Use `octenclaw voicecall ...` or the `voice_call` tool
...
```bash
octenclaw plugins install ./extensions/voice-call
...
```bash
octenclaw voicecall call --to "+15555550123" --message "Hello from OctenClaw"
octenclaw voicecall continue --call-id <id> --message "Any questions?"
octenclaw voicecall speak --call-id <id> --message "One moment"
octenclaw voicecall end --call-id <id>
octenclaw voicecall status --call-id <id>
octenclaw voicecall tail
