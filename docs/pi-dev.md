...
This guide summarizes a sane workflow for working on the pi integration in OctenClaw.
...
- Trigger the agent directly:
  - `pnpm octenclaw agent --message "Hello" --thinking low`
...
State lives under the OctenClaw state directory. Default is `~/.openclaw`. If `OPENCLAW_STATE_DIR` is set, use that directory instead.
