...
read_when:
  ...
  - Wiring external systems into OctenClaw
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your OctenClaw config directory (typically `~/.openclaw/hooks/transforms`).
...
- `octenclaw webhooks gmail setup` writes `hooks.gmail` config for `octenclaw webhooks gmail run`.
