...
- For upgrades, `octenclaw doctor --fix` can normalize legacy cron store fields before the scheduler touches them.
...
octenclaw cron list
octenclaw cron run <job-id>
octenclaw cron runs --id <job-id>
```
...
are only safe when the Gateway is stopped. Prefer `octenclaw cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, OctenClaw applies a
...
If `delivery` is omitted for isolated jobs, OctenClaw defaults to `announce`.
...
- CLI equivalents: `octenclaw cron add --light-context ...` and `octenclaw cron edit --light-context`.
...
When a job fails, OctenClaw classifies errors as **transient** (retryable) or **permanent** (disable immediately).
...
- For removed run sessions no longer referenced by the session store, OctenClaw archives transcript files and purges old deleted archives on the same retention window.
...
- review growth periodically with `octenclaw cron runs` and adjust retention before logs become large
...
octenclaw cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
octenclaw cron edit <jobId> --agent ops
octenclaw cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
octenclaw cron run <jobId>
octenclaw cron run <jobId> --due
```

`cron.run` now acknowledges once the manual run is queued, not after the job finishes. Successful queue responses look like `{ ok: true, enqueued: true, runId }`. If the job is already running or `--due` finds nothing due, the response stays `{ ok: true, ran: false, reason }`. Use `octenclaw cron runs --id <jobId>` or the `cron.runs` gateway method to inspect the eventual finished entry.

Edit an existing job (patch fields):

```bash
octenclaw cron edit <jobId> \
...
```bash
octenclaw cron edit <jobId> --exact
```
...
```bash
octenclaw system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`octenclaw system event`](/cli/system).
...
- OctenClaw applies exponential retry backoff for recurring jobs after consecutive errors:
