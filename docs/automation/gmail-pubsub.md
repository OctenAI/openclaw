---
summary: "Gmail Pub/Sub push wired into OctenClaw webhooks via gogcli"
read_when:
  - Wiring Gmail inbox triggers to OctenClaw
  - Setting up Pub/Sub push for agent wake
title: "Gmail PubSub"
---

# Gmail Pub/Sub -> OctenClaw

Goal: Gmail watch -> Pub/Sub push -> `gog gmail watch serve` -> OctenClaw webhook.

## Prereqs

- `gcloud` installed and logged in ([install guide](https://docs.cloud.google.com/sdk/docs/install-sdk)).
- `gog` (gogcli) installed and authorized for the Gmail account ([gogcli.sh](https://gogcli.sh/)).
- OctenClaw hooks enabled (see [Webhooks](/automation/webhook)).
...
Use the OctenClaw helper to wire everything together (installs deps on macOS via brew):

```bash
octenclaw webhooks gmail setup \
...
- Writes `hooks.gmail` config for `octenclaw webhooks gmail run`.
- Enables the Gmail hook preset (`hooks.presets: ["gmail"]`).

Path note: when `tailscale.mode` is enabled, OctenClaw automatically sets
...
- `--hook-url` points to OctenClaw `/hooks/gmail` (mapped; isolated run + summary to main).
- `--include-body` and `--max-bytes` control the body snippet sent to OctenClaw.

Recommended: `octenclaw webhooks gmail run` wraps the same flow and auto-renews the watch.
...
```bash
gog gmail watch status --account octenclaw@gmail.com
gog gmail history --account octenclaw@gmail.com --since <historyId>
```
...
```bash
gog gmail watch stop --account octenclaw@gmail.com
