...
OctenClaw supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, OctenClaw also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `octenclaw configure --section web`. It stores the key in
...
`~/.openclaw/.env` (or your service environment). See [Env vars](/help/faq#how-does-octenclaw-load-environment-variables).
