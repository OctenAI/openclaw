---
summary: "How OctenClaw sandboxing works: modes, scopes, workspace access, and images"
title: Sandboxing
...
OctenClaw can run **tools inside Docker containers** to reduce blast radius.
...
- Optional sandboxed browser (`agents.defaults.sandbox.browser`).
  ...
  - By default, sandbox browser containers use a dedicated Docker network (`octenclaw-sandbox-browser`) instead of the global `bridge` network.
  ...
  - noVNC observer access is password-protected by default; OctenClaw emits a short-lived token URL that serves a local bootstrap page and opens noVNC with password in URL fragment (not query/header logs).
...
OctenClaw mirrors eligible skills into the sandbox workspace (`.../skills`) so
...
- OctenClaw blocks dangerous bind sources (for example: `docker.sock`, `/etc`, `/proc`, `/sys`, `/dev`, and parent mounts that would expose them).
...
Default image: `octenclaw-sandbox:bookworm-slim`
...
`octenclaw-sandbox-common:bookworm-slim`.
...
- Use `octenclaw sandbox explain` to inspect effective sandbox mode, tool policy, and fix-it config keys.
