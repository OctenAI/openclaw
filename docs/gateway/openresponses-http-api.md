...
OctenClaw’s Gateway can serve an OpenResponses-compatible `POST /v1/responses` endpoint.
...
`octenclaw agent`), so routing/permissions/config match your Gateway.
...
- select agents with `model: "octenclaw:<agentId>"`, `model: "agent:<agentId>"`, or `x-octenclaw-agent-id`
- use `x-octenclaw-session-key` for explicit session routing
