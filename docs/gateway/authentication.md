...
OctenClaw supports OAuth and API keys for model providers. For always-on gateway
...
2. Put it on the **gateway host** (the machine running `octenclaw gateway`).
...
API keys for daemon use: `octenclaw onboard`.
...
```bash
octenclaw models auth paste-token --provider anthropic
octenclaw models auth paste-token --provider openrouter
```
...
- OctenClaw retries with the next key only for rate-limit errors (for example
...
Run `octenclaw models status` to confirm which profile is expiring. If the profile
