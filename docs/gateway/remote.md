...
- Use the macOS app’s **Remote over SSH** mode (Settings → General → “OctenClaw runs”).
...
- `octenclaw health` and `octenclaw status --deep` now reach the remote gateway via `ws://127.0.0.1:18789`.
- `octenclaw gateway {status,health,send,agent,call}` can also target the forwarded URL via `--url` when needed.
