...
read_when:
  - Generating or reviewing `octenclaw secrets apply` plans
...
This page defines the strict contract enforced by `octenclaw secrets apply`.

If a target does not match these rules, apply fails before mutating configuration.

## Plan file shape

`octenclaw secrets apply --from <plan.json>` expects a `targets` array of plan targets:
...
If apply fails with an invalid target path message, regenerate the plan with `octenclaw secrets configure` or fix the target path to a supported shape above.
