...
- During heartbeat runs, OctenClaw treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `octenclaw sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, OctenClaw skips the heartbeat run entirely (no model call).
...
```bash
octenclaw system event --text "Check for urgent follow-ups" --mode now
```
