...
OctenClaw uses Bonjour (mDNS / DNS‑SD) as a **LAN‑only convenience** to discover
...
OctenClaw supports any discovery domain; `openclaw.internal.` is just an example.
...
- `transport=gateway`
- `cliPath=<path>` (optional; absolute path to a runnable `octenclaw` entrypoint)
