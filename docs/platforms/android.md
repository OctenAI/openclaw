...
- You can run the CLI (`octenclaw`) on the gateway machine (or via SSH).
...
```bash
octenclaw nodes invoke --node "<Android Node>" --command canvas.navigate --params '{"url":"http://<gateway-hostname>.local:18789/__openclaw__/canvas/"}'
```
