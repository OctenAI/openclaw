---
summary: "OctenClaw on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up OctenClaw on DigitalOcean
  - Looking for cheap VPS hosting for OctenClaw
title: "DigitalOcean"
---

# OctenClaw on DigitalOcean

## Goal

Run a persistent OctenClaw Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
# Check service
systemctl --user status octenclaw-gateway.service

# View logs
journalctl --user -u octenclaw-gateway.service -f
```
...
```bash
octenclaw config set gateway.bind tailnet
octenclaw gateway restart
```
...
```bash
octenclaw channels login whatsapp
# Scan QR code
```
...
journalctl -u octenclaw --no-pager -n 50
```
