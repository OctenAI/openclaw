...
This plan defines how OctenClaw should support ACP coding agents in thread-capable channels (Discord first) with production-level lifecycle and recovery.
...
- OctenClaw core owns ACP control plane concerns
...
OctenClaw should not reimplement ACP transport internals in core.
OctenClaw should not rely on a pure plugin-only interception path for routing.

## North-star architecture (holy grail)

Treat ACP as a first-class control plane in OctenClaw, with pluggable runtime adapters.
...
- move ACP control-plane state to a dedicated SQLite store (WAL mode) under OctenClaw state dir
...
Core (must be in OctenClaw core):
...
- ACP event stream is normalized to OctenClaw reply chunks
...
For the first production backend (`extensions/acpx`), OctenClaw and acpx are
...
- encoded payload fields:
  - `name` (acpx named session; uses OctenClaw `sessionKey`)
...
- OctenClaw consumes ndjson events from `acpx --format json --json-strict`
