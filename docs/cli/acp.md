...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to a OctenClaw Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`octenclaw acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the OctenClaw gateway or agent instead.                                                                                                                                     |
...
# Remote Gateway
octenclaw acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
octenclaw acp --url wss://gateway-host:18789 --token-file ~/.openclaw/gateway.token

# Attach to an existing session key
octenclaw acp --session agent:main:main

# Attach by label (must already exist)
octenclaw acp --session-label "support inbox"

# Reset the session key before the first prompt
octenclaw acp --session agent:main:main --reset-session
```
...
```bash
octenclaw acp client

# Point the spawned bridge at a remote Gateway
octenclaw acp client --server-args --url wss://gateway-host:18789 --token-file ~/.openclaw/gateway.token

# Override the server command (default: octenclaw)
octenclaw acp client --server "node" --server-args openclaw.mjs acp --url ws://127.0.0.1:19001
```
...
it to drive a OctenClaw Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `octenclaw acp` over stdio.

Example config (persisted):

```bash
octenclaw config set gateway.remote.url wss://gateway-host:18789
octenclaw config set gateway.remote.token <token>
```
...
```bash
octenclaw acp --session agent:main:main
octenclaw acp --session agent:design:main
octenclaw acp --session agent:qa:bug-123
```
...
OctenClaw bot over ACP, use `acpx` with its built-in `octenclaw` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx octenclaw` at `octenclaw acp`.
3. Target the OctenClaw session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default OctenClaw ACP session
acpx octenclaw exec "Summarize the active OctenClaw session state."

# Persistent named session for follow-up turns
acpx octenclaw sessions ensure --name codex-bridge
acpx octenclaw -s codex-bridge --cwd /path/to/repo \
  "Ask my OctenClaw work agent for recent context relevant to this repo."
```

If you want `acpx octenclaw` to target a specific Gateway and session key every
time, override the `octenclaw` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "octenclaw": {
      "command": "env OPENCLAW_HIDE_BANNER=1 OPENCLAW_SUPPRESS_NOTES=1 octenclaw acp --url ws://127.0.0.1:18789 --token-file ~/.openclaw/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local OctenClaw checkout, use the direct CLI entrypoint instead of the
...
pull contextual information from an OctenClaw agent without scraping a terminal.
...
In Zed, open the Agent panel and select “OctenClaw ACP” to start a thread.
...
- `octenclaw acp client` sets `OPENCLAW_SHELL=acp-client` on the spawned bridge process.

### `acp client` options

- `--cwd <dir>`: working directory for the ACP session.
- `--server <command>`: ACP server command (default: `octenclaw`).
