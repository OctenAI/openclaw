---
summary: "CLI reference for `octenclaw onboard` (interactive onboarding wizard)"
read_when:
...
```bash
octenclaw onboard
octenclaw onboard --flow quickstart
octenclaw onboard --flow manual
octenclaw onboard --mode remote --remote-url wss://gateway-host:18789
```
...
- `--install-daemon` starts the managed gateway install path first. Without it, you must already have a local gateway running, for example `octenclaw gateway run`.
...
- Fastest first chat: `octenclaw dashboard` (Control UI, no channel setup).
