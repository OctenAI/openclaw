---
summary: "CLI reference for `octenclaw configure` (interactive configuration prompts)"
read_when:
...
Tip: `octenclaw config` without a subcommand opens the same wizard. Use
`octenclaw config get|set|unset` for non-interactive edits.
...
```bash
octenclaw configure
octenclaw configure --section model --section channels
```
