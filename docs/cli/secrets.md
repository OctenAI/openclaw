---
summary: "CLI reference for `octenclaw secrets` (reload, audit, configure, apply)"
read_when:
...
Use `octenclaw secrets` to manage SecretRefs and keep the active runtime snapshot healthy.
...
```bash
octenclaw secrets audit
octenclaw secrets audit --check
octenclaw secrets audit --json
```
...
```bash
octenclaw secrets configure
octenclaw secrets configure --plan-out /tmp/openclaw-secrets-plan.json
octenclaw secrets configure --apply --yes
octenclaw secrets configure --providers-only
octenclaw secrets configure --skip-provider-setup
octenclaw secrets configure --agent ops
octenclaw secrets configure --json
```
...
- On Windows, if ACL verification is unavailable for a provider path, OctenClaw fails closed. For trusted paths only, set `allowInsecurePath: true` on that provider to bypass path security checks.
...
octenclaw secrets apply --from /tmp/openclaw-secrets-plan.json
octenclaw secrets apply --from /tmp/openclaw-secrets-plan.json --dry-run
octenclaw secrets apply --from /tmp/openclaw-secrets-plan.json --json
```
