...
- With Anthropic API-key auth profiles, OctenClaw seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, OctenClaw injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
OctenClaw exposes dedicated cache-trace diagnostics for embedded agent runs.
