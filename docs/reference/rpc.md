...
OctenClaw integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- OctenClaw owns lifecycle when `channels.signal.autoStart=true`.
...
- OctenClaw spawns `imsg rpc` as a child process (legacy iMessage integration).
