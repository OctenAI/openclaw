...
This document explains how OctenClaw manages sessions end-to-end:
...
OctenClaw is designed around a single **Gateway process** that owns session state.
...
OctenClaw persists sessions in two layers:
...
OctenClaw resolves these via `src/config/sessions.ts`.
...
In `mode: "warn"`, OctenClaw reports potential evictions but does not mutate the store/files.
...
OctenClaw intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (OctenClaw consumes the events, but Pi decides when to compact).
...
OctenClaw also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, OctenClaw bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, OctenClaw leaves it alone.
...
- `octenclaw status` (CLI)
- `octenclaw sessions` / `sessions --json`
...
OctenClaw supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- OctenClaw strips/suppresses this in the delivery layer.

As of `2026.1.10`, OctenClaw also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
OctenClaw uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but OctenClaw’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `octenclaw status`.
