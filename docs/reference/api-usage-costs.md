...
OctenClaw features that can generate provider usage or paid API calls.
...
- `octenclaw status --usage` and `octenclaw channels list` show provider **usage windows**
...
OctenClaw can pick up credentials from:
...
- `octenclaw status --usage`
- `octenclaw models status --json`
...
`octenclaw models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
