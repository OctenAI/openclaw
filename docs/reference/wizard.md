...
This is the full reference for the `octenclaw onboard` CLI wizard.
...
<Steps>
  <Step title="Existing config detection">
    ...
    - If the config is invalid or contains legacy keys, the wizard stops and asks
      you to run `octenclaw doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via `octenclaw pairing approve <channel> <code>` or use allowlists.
  </Step>
  <Step title="Web search">
    ...
    - Configure later: `octenclaw configure --section web`.
  </Step>
  ...
  <Step title="Health check">
    - Starts the Gateway (if needed) and runs `octenclaw health`.
    - Tip: `octenclaw status --deep` adds gateway health probes to status output (requires a reachable gateway).
  </Step>
...
`octenclaw agents add` writes `agents.list[]` and optional `bindings`.
