---
summary: "How OctenClaw builds prompt context and reports token usage + costs"
read_when:
...
OctenClaw tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

OctenClaw assembles its own system prompt on every run. It includes:
...
For images, OctenClaw downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from OctenClaw session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `octenclaw status --usage` and `octenclaw channels list` show
...
`cacheWrite`. If pricing is missing, OctenClaw shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. OctenClaw can
...
Anthropic's 1M context window is currently beta-gated. OctenClaw can inject the
...
OctenClaw skips the `context-1m-*` beta header because Anthropic currently
