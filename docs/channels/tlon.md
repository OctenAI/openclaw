...
Tlon is a decentralized messenger built on Urbit. OctenClaw connects to your Urbit ship and can
...
```bash
octenclaw plugins install @openclaw/tlon
```

Local checkout (when running from a git repo):

```bash
octenclaw plugins install ./extensions/tlon
```
...
By default, OctenClaw blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `octenclaw message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, OctenClaw replies in-thread.
