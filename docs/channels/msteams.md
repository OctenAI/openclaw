...
```bash
octenclaw plugins install @openclaw/msteams
```

Local checkout (when running from a git repo):

```bash
octenclaw plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
OctenClaw will offer the local install path automatically.
...
3. Configure OctenClaw with those credentials.
...
- Talk to OctenClaw via Teams DMs, group chats, or channels.
...
- On startup, OctenClaw resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
Before configuring OctenClaw, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `octenclaw-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `octenclaw plugins install @openclaw/msteams`
   - From a local checkout: `octenclaw plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "OctenClaw in Teams", "full": "OctenClaw in Teams" },
...
OctenClaw handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, OctenClaw only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
OctenClaw sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `octenclaw message poll --channel msteams --target conversation:<id> ...`
...
```bash
octenclaw message send --channel msteams \
...
octenclaw message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
octenclaw message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
octenclaw message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
octenclaw message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
