...
- OctenClaw installed on your server (Linux flow below tested on Ubuntu 24).
...
3. Choose one setup path:
   - **Path A (QR link):** `signal-cli link -n "OctenClaw"` and scan with Signal.
   - **Path B (SMS register):** register a dedicated number with captcha + SMS verification.
4. Configure OctenClaw and restart the gateway.
5. Send a first DM and approve pairing (`octenclaw pairing approve signal <CODE>`).
...
2. Link a bot account:
   - `signal-cli link -n "OctenClaw"` then scan the QR in Signal.
...
4. Configure OctenClaw, restart gateway, verify channel:
...
5. Pair your DM sender:
   ...
   - Approve code on the server: `octenclaw pairing approve signal <PAIRING_CODE>`.
...
If you want to manage `signal-cli` yourself (slow JVM cold starts, container init, or shared CPUs), run the daemon separately and point OctenClaw at it:
...
This skips auto-spawn and the startup wait inside OctenClaw. For slow starts when auto-spawning, set `channels.signal.startupTimeoutMs`.
...
- Approve via:
  - `octenclaw pairing list signal`
  - `octenclaw pairing approve signal <CODE>`
...
- **Typing indicators**: OctenClaw sends typing signals via `signal-cli sendTyping` and refreshes them while a reply is running.
- **Read receipts**: when `channels.signal.sendReadReceipts` is true, OctenClaw forwards read receipts for allowed DMs.
...
- Config validation errors after edits: run `octenclaw doctor --fix`.
