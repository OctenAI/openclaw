...
read_when:
  - Setting up Zalo Personal for OctenClaw
...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside OctenClaw.
...
- Install via CLI: `octenclaw plugins install @openclaw/zalouser`
- Or from a source checkout: `octenclaw plugins install ./extensions/zalouser`
...
2. Login (QR, on the Gateway machine):
   - `octenclaw channels login --channel zalouser`
...
```bash
octenclaw directory self --channel zalouser
octenclaw directory peers list --channel zalouser --query "name"
octenclaw directory groups list --channel zalouser --query "work"
```
...
- `octenclaw pairing list zalouser`
- `octenclaw pairing approve zalouser <code>`
...
- On startup, OctenClaw resolves group/user names in allowlists to IDs and logs the mapping.
...
- When a group message is skipped because mention is required, OctenClaw stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in OctenClaw state. Example:
...
- OctenClaw sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, OctenClaw sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `octenclaw channels logout --channel zalouser && octenclaw channels login --channel zalouser`
...
- The channel now runs fully in OctenClaw without external CLI binaries.
