    ...
    list: [
      {
        ...
        groupChat: {
          historyLimit: 50,
          mentionPatterns: ["@?octenclaw", "\\+?15555550123"],
        },
      },
    ],
  },
...
- The regexes are case-insensitive; they cover a display-name ping like `@octenclaw` and the raw number with or without `+`/spaces.
...
1. Add your WhatsApp account (the one running OctenClaw) to the group.
2. Say `@octenclaw …` (or include the number). Only allowlisted senders can trigger it unless you set `groupPolicy: "open"`.
...
- Manual smoke:
  - Send an `@octenclaw` ping in the group and confirm a reply that references the sender name.
