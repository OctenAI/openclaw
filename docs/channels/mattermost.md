---
summary: "Mattermost bot setup and OctenClaw config"
read_when:
...
```bash
octenclaw plugins install @openclaw/mattermost
```

Local checkout (when running from a git repo):

```bash
octenclaw plugins install ./extensions/mattermost
```
...
4. Configure OctenClaw and start the gateway.
...
Native slash commands are opt-in. When enabled, OctenClaw registers `oc_*` slash commands via
...
- If `callbackUrl` is omitted, OctenClaw derives one from gateway host/port + `callbackPath`.
...
- Reachability requirement: the callback endpoint must be reachable from the Mattermost server.
  - Do not set `callbackUrl` to `localhost` unless Mattermost runs on the same host/network namespace as OctenClaw.
  - Do not set `callbackUrl` to your Mattermost base URL unless that URL reverse-proxies `/api/channels/mattermost/command` to OctenClaw.
  - A quick check is `curl https://<gateway-host>/api/channels/mattermost/command`; a GET should return `405 Method Not Allowed` from OctenClaw, not `404`.
...
- Approve via:
  - `octenclaw pairing list mattermost`
  - `octenclaw pairing approve mattermost <CODE>`
...
Use these target formats with `octenclaw message send` or cron/webhooks:
...
OctenClaw resolves them **user-first**:

- If the ID exists as a user (`GET /api/v4/users/<id>` succeeds), OctenClaw sends a **DM** by resolving the direct channel via `/api/v4/channels/direct`.
...
- If `interactions.callbackBaseUrl` is omitted, OctenClaw derives the callback URL from
...
- Reachability rule: the button callback URL must be reachable from the Mattermost server.
  `localhost` only works when Mattermost and OctenClaw run on the same host/network namespace.
...
1. Derive the secret from the bot token:
   `HMAC-SHA256(key="octenclaw-mattermost-interactions", data=botToken)`
...
secret = hmac.new(
    b"octenclaw-mattermost-interactions",
...
`octenclaw message send` and cron/webhook deliveries.
