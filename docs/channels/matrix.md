...
Matrix is an open, decentralized messaging protocol. OctenClaw connects as a Matrix **user**
...
```bash
octenclaw plugins install @openclaw/matrix
```

Local checkout (when running from a git repo):

```bash
octenclaw plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `octenclaw plugins install @openclaw/matrix`
   - From a local checkout: `octenclaw plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: OctenClaw calls the same
...
- On first connection, OctenClaw requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  OctenClaw logs a warning.
...
- Approve via:
  - `octenclaw pairing list matrix`
  - `octenclaw pairing approve matrix <CODE>`
...
- On startup, OctenClaw resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
