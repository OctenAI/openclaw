...
```bash
octenclaw plugins install @openclaw/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
octenclaw plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "OctenClaw" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `octenclaw pairing list nextcloud-talk`
  - `octenclaw pairing approve nextcloud-talk <CODE>`
