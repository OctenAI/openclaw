...
read_when:
  - Setting up Twitch chat integration for OctenClaw
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. OctenClaw connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
octenclaw plugins install @openclaw/twitch
```

Local checkout (when running from a git repo):

```bash
octenclaw plugins install ./extensions/twitch
```
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "octenclaw", // Bot's Twitch account
