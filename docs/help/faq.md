---
summary: "Frequently asked questions about OctenClaw setup, configuration, and usage"
read_when:
...
- [Quick start and first-run setup]
  ...
  - [What's the recommended way to install and set up OctenClaw?](#whats-the-recommended-way-to-install-and-set-up-octenclaw)
  ...
  - [Windows install says git not found or octenclaw not recognized](#windows-install-says-git-not-found-or-octenclaw-not-recognized)
  ...
  - [How do I install OctenClaw on Linux?](#how-do-i-install-octenclaw-on-linux)
  - [How do I install OctenClaw on a VPS?](#how-do-i-install-octenclaw-on-a-vps)
  - [Where are the cloud/VPS install guides?](#where-are-the-cloudvps-install-guides)
  - [Can I ask OctenClaw to update itself?](#can-i-ask-octenclaw-to-update-itself)
  ...
  - [If I buy a Mac mini to run OctenClaw, can I connect it to my MacBook Pro?](#if-i-buy-a-mac-mini-to-run-octenclaw-can-i-connect-it-to-my-macbook-pro)
  - [Can I use Bun?](#can-i-use-bun)
  - [Telegram: what goes in `allowFrom`?](#telegram-what-goes-in-allowfrom)
  - [Can multiple people use one WhatsApp number with different OctenClaw instances?](#can-multiple-people-use-one-whatsapp-number-with-different-octenclaw-instances)
  - [Can I run a "fast chat" agent and an "Opus for coding" agent?](#can-i-run-a-fast-chat-agent-and-an-opus-for-coding-agent)
  - [Does Homebrew work on Linux?](#does-homebrew-work-on-linux)
  - [What's the difference between the hackable (git) install and npm install?](#whats-the-difference-between-the-hackable-git-install-and-npm-install)
  - [Can I switch between npm and git installs later?](#can-i-switch-between-npm-and-git-installs-later)
  - [Should I run the Gateway on my laptop or a VPS?](#should-i-run-the-gateway-on-my-laptop-or-a-vps)
  - [How important is it to run OctenClaw on a dedicated machine?](#how-important-is-it-to-run-octenclaw-on-a-dedicated-machine)
  - [What are the minimum VPS requirements and recommended OS?](#what-are-the-minimum-vps-requirements-and-recommended-os)
  - [Can I run OctenClaw in a VM and what are the requirements](#can-i-run-octenclaw-in-a-vm-and-what-are-the-requirements)
- [What is OctenClaw?](#what-is-octenclaw)
  - [What is OctenClaw, in one paragraph?](#what-is-octenclaw-in-one-paragraph)
  - [What's the value proposition?](#whats-the-value-proposition)
  - [I just set it up what should I do first](#i-just-set-it-up-what-should-i-do-first)
  - [What are the top five everyday use cases for OctenClaw](#what-are-the-top-five-everyday-use-cases-for-octenclaw)
  - [Can OctenClaw help with lead gen outreach ads and blogs for a SaaS](#can-octenclaw-help-with-lead-gen-outreach-ads-and-blogs-for-a-saas)
...
- [Skills and automation](#skills-and-automation)
  ...
  - [Can OctenClaw run tasks on a schedule or continuously in the background?](#can-octenclaw-run-tasks-on-a-schedule-or-continuously-in-the-background)
...
- [Where things live on disk](#where-things-live-on-disk)
  - [Is all data used with OctenClaw saved locally?](#is-all-data-used-with-octenclaw-saved-locally)
  - [Where does OctenClaw store its data?](#where-does-octenclaw-store-its-data)
  - [Where should AGENTS.md / SOUL.md / USER.md / MEMORY.md live?](#where-should-agentsmd-soulmd-usermd-memorymd-live)
  - [What's the recommended backup strategy?](#whats-the-recommended-backup-strategy)
  - [How do I completely uninstall OctenClaw?](#how-do-i-completely-uninstall-octenclaw)
...
- [Config basics](#config-basics)
  ...
  - [Can the OctenClaw browser run headless?](#can-the-octenclaw-browser-run-headless)
...
- [Remote gateways and nodes](#remote-gateways-and-nodes)
  ...
  - [Can two OctenClaw instances talk to each other (local + VPS)?](#can-two-octenclaw-instances-talk-to-each-other-local-vps)
...
- [Env vars and .env loading](#env-vars-and-env-loading)
  - [How does OctenClaw load environment variables?](#how-does-octenclaw-load-environment-variables)
...
- [Sessions and multiple chats](#sessions-and-multiple-chats)
  ...
  - [Is there a way to make a team of OctenClaw instances one CEO and many agents](#is-there-a-way-to-make-a-team-of-octenclaw-instances-one-ceo-and-many-agents)
  - [Why did context get truncated mid-task? How do I prevent it?](#why-did-context-get-truncated-midtask-how-do-i-prevent-it)
  - [How do I completely reset OctenClaw but keep it installed?](#how-do-i-completely-reset-octenclaw-but-keep-it-installed)
  ...
  - [Why doesn't OctenClaw reply in a group?](#why-doesnt-octenclaw-reply-in-a-group)
...
- [Models: defaults, selection, aliases, switching](#models-defaults-selection-aliases-switching)
  ...
  - [What do OctenClaw, Flawd, and Krill use for models?](#what-do-octenclaw-flawd-and-krill-use-for-models)
...
- [Gateway: ports, "already running", and remote mode](#gateway-ports-already-running-and-remote-mode)
  ...
  - [Why does `octenclaw gateway status` say `Runtime: running` but `RPC probe: failed`?](#why-does-octenclaw-gateway-status-say-runtime-running-but-rpc-probe-failed)
  - [Why does `octenclaw gateway status` show `Config (cli)` and `Config (service)` different?](#why-does-octenclaw-gateway-status-show-config-cli-and-config-service-different)
  - [What does "another gateway instance is already listening" mean?](#what-does-another-gateway-instance-is-already-listening-mean)
  - [How do I run OctenClaw in remote mode (client connects to a Gateway elsewhere)?](#how-do-i-run-octenclaw-in-remote-mode-client-connects-to-a-gateway-elsewhere)
...
- [Logging and debugging](#logging-and-debugging)
  ...
  - [I closed my terminal on Windows - how do I restart OctenClaw?](#i-closed-my-terminal-on-windows-how-do-i-restart-octenclaw)
  ...
  - [ELI5: `octenclaw gateway restart` vs `octenclaw gateway`](#eli5-octenclaw-gateway-restart-vs-octenclaw-gateway)
  - [What's the fastest way to get more details when something fails?](#whats-the-fastest-way-to-get-more-details-when-something-fails)
- [Media and attachments](#media-and-attachments)
  - [My skill generated an image/PDF, but nothing was sent](#my-skill-generated-an-imagepdf-but-nothing-was-sent)
- [Security and access control](#security-and-access-control)
  - [Is it safe to expose OctenClaw to inbound DMs?](#is-it-safe-to-expose-octenclaw-to-inbound-dms)
...
7. **Gateway snapshot**
   ...
   ```bash
   octenclaw health --json
   octenclaw health --verbose   # shows the target URL + config path on errors
   ```
...
This installs OctenClaw **from a git checkout**, so the agent can read the code + docs and
...
```bash
octenclaw status
octenclaw models status
octenclaw doctor
```

What they do:

- `octenclaw status`: quick snapshot of gateway/agent health + basic config.
- `octenclaw models status`: checks provider auth + model availability.
- `octenclaw doctor`: validates and repairs common config/state issues.

Other useful CLI checks: `octenclaw status --all`, `octenclaw logs --follow`,
`octenclaw gateway status`, `octenclaw health --verbose`.

Quick debug loop: [First 60 seconds if something's broken](#first-60-seconds-if-somethings-broken).
Install docs: [Install](/install), [Installer flags](/install/installer), [Updating](/install/updating).

### What's the recommended way to install and set up OctenClaw
...
If you don't have a global install yet, run it via `pnpm octenclaw onboard`.
...
- Retrieve it from the gateway host: `octenclaw config get gateway.auth.token` (or generate one: `octenclaw doctor --generate-gateway-token`).

**Not on localhost:**

- **Tailscale Serve** (recommended): keep bind loopback, run `octenclaw gateway --tailscale serve`, open `https://<magicdns>/`. If `gateway.auth.allowTailscale` is `true`, identity headers satisfy Control UI/WebSocket auth (no token, assumes trusted gateway host); HTTP APIs still require token/password.
- **Tailnet bind**: run `octenclaw gateway --bind tailnet --token "<token>"`, open `http://<tailscale-ip>:18789/`, paste token in dashboard settings.
...
1. Install OctenClaw on the new machine.
2. Copy `$OPENCLAW_STATE_DIR` (default: `~/.openclaw`) from the old machine.
3. Copy your workspace (default: `~/.openclaw/workspace`).
4. Run `octenclaw doctor` and restart the Gateway service.
...
Related: [Migrating](/install/migrating), [Where things live on disk](/help/faq#where-does-octenclaw-store-its-data),
...
### Windows install says git not found or octenclaw not recognized
...
**2) octenclaw is not recognized after install**
...
If you still reproduce this on latest OctenClaw, track/report it in:
...
### How do I install OctenClaw on Linux
...
### How do I install OctenClaw on a VPS
...
### Can I ask OctenClaw to update itself
...
```bash
octenclaw update
octenclaw update status
octenclaw update --channel stable|beta|dev
octenclaw update --tag <dist-tag|version>
octenclaw update --no-restart
```

If you must automate from an agent:

```bash
octenclaw update --yes --no-restart
octenclaw gateway restart
```
...
`octenclaw onboard` is the recommended setup path. In **local mode** it walks you through:
...
No. You can run OctenClaw with **API keys** (Anthropic/OpenAI/others) or with
...
OpenAI Codex OAuth is explicitly supported for external tools like OctenClaw.
...
`claude setup-token` generates a **token string** via the Claude Code CLI (it is not available in the web console). You can run it on **any machine**. Choose **Anthropic token (paste setup-token)** in the wizard or paste it with `octenclaw models auth paste-token --provider anthropic`. The token is stored as an auth profile for the **anthropic** provider and used like an API key (no auto-refresh). More detail: [OAuth](/concepts/oauth).
...
Copy the token it prints, then choose **Anthropic token (paste setup-token)** in the wizard. If you want to run it on the gateway host, use `octenclaw models auth setup-token --provider anthropic`. If you ran `claude setup-token` elsewhere, paste it on the gateway host with `octenclaw models auth paste-token --provider anthropic`. See [Anthropic](/providers/anthropic).

### Do you support Claude subscription auth (Claude Pro or Max)

Yes - via **setup-token**. OctenClaw no longer reuses Claude Code CLI OAuth tokens; use a setup-token or an Anthropic API key. Generate the token anywhere and paste it on the gateway host. See [Anthropic](/providers/anthropic) and [OAuth](/concepts/oauth).
...
Tip: set a **fallback model** so OctenClaw can keep replying while a provider is rate-limited.
...
OctenClaw supports **OpenAI Code (Codex)** via OAuth (ChatGPT sign-in). The wizard can run the OAuth flow and will set the default model to `openai-codex/gpt-5.4` when appropriate. See [Model providers](/concepts/model-providers) and [Wizard](/start/wizard).

### Do you support OpenAI subscription auth Codex OAuth

Yes. OctenClaw fully supports **OpenAI Code (Codex) subscription OAuth**.
OpenAI explicitly allows subscription OAuth usage in external tools/workflows
like OctenClaw. The onboarding wizard can run the OAuth flow for you.
...
1. Enable the plugin: `octenclaw plugins enable google-gemini-cli-auth`
2. Login: `octenclaw models auth login --provider google-gemini-cli --set-default`

This stores OAuth tokens in auth profiles on the gateway host. Details: [Model providers](/concepts/model-providers).

### Is a local model OK for casual chats

Usually no. OctenClaw needs large context + strong safety; small cards truncate and leak. If you must, run the **largest** MiniMax M2.5 build you can locally (LM Studio) and see [/gateway/local-models](/gateway/local-models). Smaller/quantized models increase prompt-injection risk - see [Security](/gateway/security).
...
No. OctenClaw runs on macOS or Linux (Windows via WSL2). A Mac mini is optional - some people
...
### If I buy a Mac mini to run OctenClaw can I connect it to my MacBook Pro
...
- Use `octenclaw nodes status` / `octenclaw nodes list` to see it.
...
The onboarding wizard accepts `@username` input and resolves it to a numeric ID, but OctenClaw authorization uses numeric IDs only.

Safer (no third-party bot):

- DM your bot, then run `octenclaw logs --follow` and read `from.id`.
...
### Can multiple people use one WhatsApp number with different OctenClaw instances
...
If you run OctenClaw via systemd, ensure the service PATH includes `/home/linuxbrew/.linuxbrew/bin` (or your brew prefix) so `brew`-installed tools resolve in non-login shells.
...
This **does not delete your data** - it only changes the OctenClaw code install. Your state
...
**OctenClaw-specific note:** WhatsApp/Telegram/Slack/Mattermost (plugin)/Discord all work fine from a VPS. The only real trade-off is **headless browser** vs a visible window. See [Browser](/tools/browser).

**Recommended default:** VPS if you had gateway disconnects before. Local is great when you're actively using the Mac and want local file access or UI automation with a visible browser.

### How important is it to run OctenClaw on a dedicated machine
...
OctenClaw is lightweight. For a basic Gateway + one chat channel:
...
### Can I run OctenClaw in a VM and what are the requirements
...
### What is OctenClaw in one paragraph

OctenClaw is a personal AI assistant you run on your own devices. It replies on the messaging surfaces you already use (WhatsApp, Telegram, Slack, Mattermost (plugin), Discord, Google Chat, Signal, iMessage, WebChat) and can also do voice + a live Canvas on supported platforms. The **Gateway** is the always-on control plane; the assistant is the product.

### What's the value proposition

OctenClaw is not "just a Claude wrapper." It's a **local-first control plane** that lets you run a
...
### What are the top five everyday use cases for OctenClaw
...
### Can OctenClaw help with lead gen outreach ads and blogs for a SaaS
...
OctenClaw draft and you approve.

Docs: [Security](/gateway/security).

### What are the advantages vs Claude Code for web development

OctenClaw is a **personal assistant** and coordination layer, not an IDE replacement. Use
Claude Code or Codex for the fastest direct coding loop inside a repo. Use OctenClaw when you
...
Yes. Add extra directories via `skills.load.extraDirs` in `~/.openclaw/openclaw.json` (lowest precedence). Default precedence remains: `<workspace>/skills` → `~/.openclaw/skills` → bundled → `skills.load.extraDirs`. `clawhub` installs into `./skills` by default, which OctenClaw treats as `<workspace>/skills`.
...
```bash
octenclaw cron run <jobId> --force
octenclaw cron runs --id <jobId> --limit 50
```
...
### Can OctenClaw run tasks on a schedule or continuously in the background
...
Run the Gateway where the macOS binaries exist, then connect from Linux in [remote mode](#how-do-i-run-octenclaw-in-remote-mode-client-connects-to-a-gateway-elsewhere) or over Tailscale. The skills load normally because the Gateway host is macOS.

**Option B - use a macOS node (no SSH).**
Run the Gateway on Linux, pair a macOS node (menubar app), and set **Node Run Commands** to "Always Ask" or "Always Allow" on the Mac. OctenClaw can treat macOS-only skills as eligible when the required binaries exist on the node. The agent runs those skills via the `nodes` tool. If you choose "Always Ask", approving "Always Allow" in the prompt adds that command to the allowlist.
...
3. Override the skill metadata (workspace or `~/.openclaw/skills`) to allow Linux:
   ...
   metadata: { "octenclaw": { "os": ["darwin", "linux"], "requires": { "bins": ["memo"] } } }
   ---
   ```
...
ClawHub installs into `./skills` under your current directory (or falls back to your configured OctenClaw workspace); OctenClaw treats that as `<workspace>/skills` on the next session. For shared skills across agents, place them in `~/.openclaw/skills/<name>/SKILL.md`. Some skills expect binaries installed via Homebrew; on Linux that means Linuxbrew (see the Homebrew Linux FAQ entry above). See [Skills](/tools/skills) and [ClawHub](/tools/clawhub).
...
OctenClaw memory is just Markdown files in the agent workspace:

- Daily notes in `memory/YYYY-MM-DD.md`
- Curated long-term notes in `MEMORY.md` (main/private sessions only)

OctenClaw also runs a **silent pre-compaction memory flush** to remind the model
...
If you don't set a provider explicitly, OctenClaw auto-selects a provider when it
...
configured and present, OctenClaw
...
### Is all data used with OctenClaw saved locally

No - **OctenClaw's state is local**, but **external services still see what you send them**.
...
### Where does OctenClaw store its data
...
Legacy single-agent path: `~/.openclaw/agent/*` (migrated by `octenclaw doctor`).
...
### How do I completely uninstall OctenClaw
...
`workspace` to the repo root. The OctenClaw repo is just source code; keep the
...
OctenClaw reads an optional **JSON5** config from `$OPENCLAW_CONFIG_PATH` (default: `~/.openclaw/openclaw.json`):
...
OctenClaw enforces token auth by default, including loopback. If no token is configured, gateway startup auto-generates one and saves it to `gateway.auth.token`, so **local WS clients must authenticate**. This blocks other local processes from calling the Gateway.

If you **really** want open loopback, set `gateway.auth.mode: "none"` explicitly in your config. Doctor can generate a token for you any time: `octenclaw doctor --generate-gateway-token`.
...
- `default`: uses `All your chats, one OctenClaw.` every time.
...
**Recommended:** run `octenclaw configure --section web` and choose a provider.
...
### Can the OctenClaw browser run headless
...
5. Approve the node on the Gateway:
   ...
   ```bash
   octenclaw devices list
   octenclaw devices approve <requestId>
   ```
...
- Gateway is running: `octenclaw gateway status`
- Gateway health: `octenclaw status`
- Channel health: `octenclaw channels status`
...
### Can two OctenClaw instances talk to each other local VPS
...
`octenclaw agent --message ... --deliver`, targeting a chat where the other bot
...
```bash
octenclaw agent --message "Hello from local bot" --deliver --channel telegram --reply-to <chat-id>
```
...
- If you have no backup, re-run `octenclaw doctor` and reconfigure channels/models.
- If this was unexpected, file a bug and include your last known config or any backup.
- A local coding agent can often reconstruct a working config from logs or history.

Avoid it:

- Use `octenclaw config set` for small changes.
- Use `octenclaw configure` for interactive edits.
...
```bash
octenclaw gateway --tailscale serve
```
...
### How does OctenClaw load environment variables

OctenClaw reads env vars from the parent process (shell, launchd/systemd, CI, etc.) and additionally loads:
...
`octenclaw models status` reports whether **shell env import** is enabled. "Shell env: off"
does **not** mean your env vars are missing - it just means OctenClaw won't load
...
### Is there a way to make a team of OctenClaw instances one CEO and many agents
...
### How do I completely reset OctenClaw but keep it installed
...
```bash
octenclaw reset --scope full --yes --non-interactive
```
...
- Dev reset: `octenclaw gateway --dev --reset` (dev-only; wipes dev config + credentials + sessions + workspace).
...
headers like `# Heading`), OctenClaw skips the heartbeat run to save API calls.
...
No. OctenClaw runs on **your own account**, so if you're in the group, OctenClaw can see it.
...
```bash
octenclaw directory groups list --channel whatsapp
```

Docs: [WhatsApp](/channels/whatsapp), [Directory](/cli/directory), [Logs](/cli/logs).

### Why doesn't OctenClaw reply in a group
...
- Use `octenclaw doctor` to spot stray workspaces and profile mismatches.
...
OctenClaw's default model is whatever you set as:

```
agents.defaults.model.primary
```

Models are referenced as `provider/model` (example: `anthropic/claude-opus-4-6`). If you omit the provider, OctenClaw currently assumes `anthropic` as a temporary deprecation fallback - but you should still **explicitly** set `provider/model`.
...
4. Run `octenclaw onboard` and choose `Ollama`
...
- for manual switching, use `octenclaw models list` and `octenclaw models set ollama/<model>`
...
- `octenclaw models set ...` (updates just model config)
- `octenclaw configure --section model` (interactive)
- edit `agents.defaults.model` in `~/.openclaw/openclaw.json`

Avoid `config.apply` with a partial object unless you intend to replace the whole config.
If you did overwrite config, restore from backup or re-run `octenclaw doctor` to repair.

Docs: [Models](/concepts/models), [Configure](/cli/configure), [Config](/cli/config), [Doctor](/gateway/doctor).

### What do OctenClaw, Flawd, and Krill use for models

- These deployments can differ and may change over time; there is no fixed provider recommendation.
- Check the current runtime setting on each gateway with `octenclaw models status`.
...
Yes. OctenClaw ships a few default shorthands (only applied when the model exists in `agents.defaults.models`):
...
- Run `octenclaw agents add <id>` and configure auth during the wizard.
...
Cooldowns apply to failing profiles (exponential backoff), so OctenClaw can keep responding even when a provider is rate-limited or temporarily failing.
...
- **Confirm where auth profiles live** (new vs legacy paths)
  ...
  - Legacy: `~/.openclaw/agent/*` (migrated by `octenclaw doctor`)
...
- **Sanity-check model/auth status**
  - Use `octenclaw models status` to see configured models and whether providers are authenticated.
...
- **Use a setup-token**
  - Run `claude setup-token`, then paste it with `octenclaw models auth setup-token --provider anthropic`.
  - If the token was created on another machine, use `octenclaw models auth paste-token --provider anthropic`.
- **If you want to use an API key instead**
  - Put `ANTHROPIC_API_KEY` in `~/.openclaw/.env` on the **gateway host**.
  - Clear any pinned order that forces a missing profile:

    ```bash
    octenclaw models auth order clear --provider anthropic
    ```
...
If your model config includes Google Gemini as a fallback (or you switched to a Gemini shorthand), OctenClaw will try it during model fallback. If you haven't configured Google credentials, you'll see `No API key found for provider "google"`.
...
Fix: OctenClaw now strips unsigned thinking blocks for Google Antigravity Claude. If it still appears, start a **new session** or set `/thinking off` for that agent.
...
OctenClaw uses provider-prefixed IDs like:
...
OctenClaw may temporarily skip a profile if it's in a short **cooldown** (rate limits/timeouts/auth failures) or a longer **disabled** state (billing/insufficient credits). To inspect this, run `octenclaw models status --json` and check `auth.unusableProfiles`. Tuning: `auth.cooldowns.billingBackoffHours*`.

You can also set a **per-agent** order override (stored in that agent's `auth-profiles.json`) via the CLI:

```bash
# Defaults to the configured default agent (omit --agent)
octenclaw models auth order get --provider anthropic

# Lock rotation to a single profile (only try this one)
octenclaw models auth order set --provider anthropic anthropic:default

# Or set an explicit order (fallback within provider)
octenclaw models auth order set --provider anthropic anthropic:work anthropic:default
...
```bash
octenclaw models auth order set --provider anthropic --agent main anthropic:default
```

### OAuth vs API key what's the difference

OctenClaw supports both:
...
### Why does octenclaw gateway status say Runtime running but RPC probe failed

Because "running" is the **supervisor's** view (launchd/systemd/schtasks). The RPC probe is the CLI actually connecting to the gateway WebSocket and calling `status`.

Use `octenclaw gateway status` and trust these lines:

- `Probe target:` (the URL the probe actually used)
- `Listening:` (what's actually bound on the port)
- `Last gateway error:` (common root cause when the process is alive but the port isn't listening)

### Why does octenclaw gateway status show Config cli and Config service different
...
```bash
octenclaw gateway install --force
```
...
OctenClaw enforces a runtime lock by binding the WebSocket listener immediately on startup (default `ws://127.0.0.1:18789`). If the bind fails with `EADDRINUSE`, it throws `GatewayLockError` indicating another instance is already listening.

Fix: stop the other instance, free the port, or run with `octenclaw gateway --port <port>`.

### How do I run OctenClaw in remote mode client connects to a Gateway elsewhere
...
- `octenclaw gateway` only starts when `gateway.mode` is `local` (or you pass the override flag).
...
- Fastest: `octenclaw dashboard` (prints + copies the dashboard URL, tries to open; shows SSH hint if headless).
- If you don't have a token yet: `octenclaw doctor --generate-gateway-token`.
...
- If mismatch persists after the one retry, rotate/re-approve the paired device token:
  - `octenclaw devices list`
  - `octenclaw devices rotate --device <id> --role operator`
- Still stuck? Run `octenclaw status --all` and follow [Troubleshooting](/gateway/troubleshooting). See [Dashboard](/web/dashboard) for auth details.
...
- Use `octenclaw --profile <name> …` per instance (auto-creates `~/.openclaw-<name>`).
- Set a unique `gateway.port` in each profile config (or pass `--port` for manual runs).
- Install a per-profile service: `octenclaw --profile <name> gateway install`.

Profiles also suffix service names (`ai.openclaw.<profile>`; legacy `com.openclaw.*`, `octenclaw-gateway-<profile>.service`, `OctenClaw Gateway (<profile>)`).
...
```
octenclaw tui --url ws://<host>:18789 --token <token>
```
...
- Linux: `journalctl --user -u octenclaw-gateway[-<profile>].service -n 200 --no-pager`
- Windows: `schtasks /Query /TN "OctenClaw Gateway (<profile>)" /V /FO LIST`
...
If you run the gateway manually, `octenclaw gateway --force` can reclaim the port. See [Gateway](/gateway).

### I closed my terminal on Windows how do I restart OctenClaw
...
1. Is the Gateway running? `octenclaw gateway status`
2. Is the Gateway healthy? `octenclaw status`
3. Does the UI have the right token? `octenclaw dashboard`
...
```bash
octenclaw channels status
octenclaw channels logs --channel telegram
```

Then match the error:

- `BOT_COMMANDS_TOO_MUCH`: the Telegram menu has too many entries. OctenClaw already trims to the Telegram limit and retries with fewer commands, but some menu entries still need to be dropped. Reduce plugin/skill/custom commands, or disable `channels.telegram.commands.native` if you do not need the menu.
...
### ELI5 octenclaw gateway restart vs octenclaw gateway

- `octenclaw gateway restart`: restarts the **background service** (launchd/systemd).
- `octenclaw gateway`: runs the gateway **in the foreground** for this terminal session.

If you installed the service, use the gateway commands. Use `octenclaw gateway` when
...
Outbound attachments from the agent must include a `MEDIA:<path-or-url>` line (on its own line). See [OctenClaw assistant setup](/start/openclaw) and [Agent send](/tools/agent-send).

CLI sending:

```bash
octenclaw message send --target +15555550123 --message "Here you go" --media /path/to/file.png
```
...
### Is it safe to expose OctenClaw to inbound DMs

Treat inbound DMs as untrusted input. Defaults are designed to reduce risk:

- Default behavior on DM-capable channels is **pairing**:
  - Unknown senders receive a pairing code; the bot does not process their message.
  - Approve with: `octenclaw pairing approve --channel <channel> [--account <id>] <code>`
  - Pending requests are capped at **3 per channel**; check `octenclaw pairing list --channel <channel> [--account <id>]` if a code didn't arrive.
- Opening DMs publicly requires explicit opt-in (`dmPolicy: "open"` and allowlist `"*"`).

Run `octenclaw doctor` to surface risky DM policies.
...
```bash
octenclaw pairing list telegram
```
...
No. Default WhatsApp DM policy is **pairing**. Unknown senders only get a pairing code and their message is **not processed**. OctenClaw only replies to chats it receives or to explicit sends you trigger.

Approve pairing with:

```bash
octenclaw pairing approve whatsapp <code>
```

List pending requests:

```bash
octenclaw pairing list whatsapp
```
...
OctenClaw blocks **cross-provider** messaging by default. If a tool call is bound
