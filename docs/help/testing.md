...
OctenClaw has three Vitest suites (unit/integration, e2e, live) and a small set of Docker runners.
...
- Pool note:
  - OctenClaw uses Vitest `vmForks` on Node 22, 23, and 24 for faster unit shards.
  - On Node 25+, OctenClaw automatically falls back to regular `forks` until the repo is re-validated there.
...
### Layer 2: Gateway + dev agent smoke (what “@octenclaw” actually does)
...
```bash
octenclaw models auth paste-token --provider anthropic --profile-id anthropic:setup-token-test
...
- Gemini API vs Gemini CLI:
  - API: OctenClaw calls Google’s hosted Gemini API over HTTP (API key / profile auth); this is what most users mean by “Gemini”.
  - CLI: OctenClaw shells out to a local `gemini` binary; it has its own auth and can behave differently (streaming/tool support/version skew).
...
- OpenRouter: `openrouter/...` (hundreds of models; use `octenclaw models scan` to find tool+image capable candidates)
...
- If a live test says “no creds”, debug the same way you’d debug `octenclaw models list` / model selection.
