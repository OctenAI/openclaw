...
OctenClaw supports **camera capture** for agent workflows:
...
```bash
octenclaw nodes camera snap --node <id>               # default: both front + back (2 MEDIA lines)
octenclaw nodes camera snap --node <id> --facing front
octenclaw nodes camera clip --node <id> --duration 3000
octenclaw nodes camera clip --node <id> --no-audio
```
...
Use the main `octenclaw` CLI to invoke camera commands on the macOS node.

Examples:

```bash
octenclaw nodes camera list --node <id>            # list camera ids
octenclaw nodes camera snap --node <id>            # prints MEDIA:<path>
octenclaw nodes camera snap --node <id> --max-width 1280
octenclaw nodes camera snap --node <id> --delay-ms 2000
octenclaw nodes camera snap --node <id> --device-id <id>
octenclaw nodes camera clip --node <id> --duration 10s          # prints MEDIA:<path>
octenclaw nodes camera clip --node <id> --duration-ms 3000      # prints MEDIA:<path> (legacy flag)
octenclaw nodes camera clip --node <id> --device-id <id>
octenclaw nodes camera clip --node <id> --no-audio
```

Notes:

- `octenclaw nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
```bash
octenclaw nodes screen record --node <id> --duration 10s --fps 15   # prints MEDIA:<path>
```
