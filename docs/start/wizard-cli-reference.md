...
read_when:
  - You need detailed behavior for octenclaw onboard
...
This page is the full reference for `octenclaw onboard`.
...
<Steps>
  <Step title="Existing config detection">
    ...
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `octenclaw doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `octenclaw pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `octenclaw health`.
    - `octenclaw status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
- Command reference: [`octenclaw onboard`](/cli/onboard)
